  ...
  <v-app :theme="theme">
    <v-app-bar  elevation="0" class="glass-effect">
      <v-toolbar-title class="text-h5 font-weight-bold">
        <v-icon large color="secondary" class="mr-2 animate-bounce">mdi-leaf</v-icon>
        
        <span class="gradient-text">Coconote</span>
      </v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn v-if="!isLoggedIn" @click="login" text color="secondary" class="text-capitalize font-weight-bold">
        <v-icon left>mdi-login</v-icon>
        로그인
      </v-btn>
      <v-btn v-if="isLoggedIn" @click="logout" text color="secondary" class="text-capitalize font-weight-bold">
      ...
      <v-container fluid class="pa-2">
        <v-row align="center" justify="center" class="ma-0">
          <v-col cols="12" sm="10" md="8" lg="6" class="pa-4">
            <v-card elevation="10" class="text-center pa-6 rounded-xl glass-card" style="min-height: 400px;">
              ...
              <v-card-text class="text-body-1 mb-6">
                <p class="mb-4">노션의 문서 관리와 슬랙의 실시간 커뮤니케이션을 하나로.</p>
                <p>팀워크의 효율성을 극대화하세요.</p>
              </v-card-text>
              <v-btn :to="isLoggedIn ? '/workspace' : '/login'" x-large rounded color="secondary"
                class="px-8 py-1 text-capitalize elevation-8 pulse-animation centered-btn" depressed>
        ...
        <v-row class="ma-0 pa-4">
          <v-col cols="12">
            ...
            <br>
            <v-row>
              <v-col v-for="feature in features" :key="feature.title" cols="12" md="4" class="mb-4">
                <v-card elevation="10" class="text-center pa-6 rounded-xl glass-card ma-t-4 ma-l-4">
                  <v-card-title class="text-h6">
                    <v-icon large left color="secondary">{{ feature.icon }}</v-icon>
                    {{ feature.title }}
                  </v-card-title>
        ...
        <v-row class="ma-0 pa-4 justify-center">
          <v-col cols="12" md="10" class="d-flex flex-column align-items-center">
            <h2 class="text-h4 mb-6 text-center">최근 업데이트</h2> <!-- 제목 중앙 정렬 -->
            <v-timeline align-top dense>
              <v-timeline-item v-for="update in recentUpdates" :key="update.date" color="secondary">
                <v-card class="glass-card" style="max-width: 700px; margin: 0 auto;">
                  <v-card-title class="text-h6">{{ update.title }}</v-card-title>
                  <v-card-text>
                    {{ update.description }}
...
const features = [
  { title: '실시간 협업', icon: 'mdi-account-group', description: '팀원들과 실시간으로 문서를 편집하고 의견을 나눌 수 있습니다.' },
  { title: '통합 커뮤니케이션', icon: 'mdi-chat', description: '문서 내 채팅, 음성 및 화상 통화 기능으로 원활한 소통이 가능합니다.' },
  { title: '강력한 문서 관리', icon: 'mdi-file-document', description: '직관적인 문서 구조화와 버전 관리 기능을 제공합니다.' },
]

const recentUpdates = [
  { title: '쓰레드 기능 추가', description: '쓰레드를 통해 자유롭게 커뮤니케이션 할 수 있습니다.', date: '2024-10-01' },
  { title: '캔버스 기능 추가', description: '캔버스를 통해 마크다운 형식으로 문서를 공유할수 있습니다..', date: '2024-10-01' },
  { title: '드라이브 기능 추가', description: '드라이블 통해 파일 이미지 등의 자료를 서로 공유 할수 있습니다.', date: '2024-10-01' },
]
...
/* 버튼 및 텍스트의 가독성 개선 */
* Button with a modern,
neutral color palette */ .v-btn {
...
* Typography adjustments for a clean look */ .text-h6,
.text-h5,
.text-h4,
.text-h2 {
